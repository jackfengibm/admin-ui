  ...
  context 'Single values' do
    ...
    context 'Values loaded and returned' do
      ...
      it 'converts stats_refresh_time to stats_refresh_schedules' do
        schedule_minutes = 30
        stats_refresh_schedules = ["#{schedule_minutes} * * * *"]
      ...
      it 'stats_refresh_schedules for predefined schedule - @mothly' do
        stats_refresh_schedules = ['@mothly']
        config = AdminUI::Config.load('stats_refresh_schedules' => stats_refresh_schedules)
        expect(config.stats_refresh_schedules).to eq([ "@mothly"])
      end
  ...
  context 'Errors' do
    ...
    context 'Invalid value types' do
      ...
      it 'stats_refresh_schedules' do
        expect { AdminUI::Config.load(config.merge(:stats_refresh_schedules => ['hi'])) }.to raise_error(Membrane::SchemaValidationError)
      end

      it 'stats_refresh_schedules with specs not compliant to crontab format - extra commma' do
        expect { AdminUI::Config.load(config.merge(:stats_refresh_schedules => ['0 1,,2 * * *', '0 12-17 * * 1-5'])) }.to raise_error(Membrane::SchemaValidationError)
      end

      it 'stats_refresh_schedules with specs not compliant to crontab format - extra range symbal' do
        expect { AdminUI::Config.load(config.merge(:stats_refresh_schedules => ['0 1--2 * * *', '0 12-17 * * 1-5'])) }.to raise_error(Membrane::SchemaValidationError)
      end

      it 'stats_refresh_schedules with specs not compliant to crontab format - use of step' do
        expect { AdminUI::Config.load(config.merge(:stats_refresh_schedules => ['0 /2 * * *', '0 12-17 * * 1-5'])) }.to raise_error(Membrane::SchemaValidationError)
      end
