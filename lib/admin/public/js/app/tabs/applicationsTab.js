...
ApplicationsTab.prototype.getTableData = function(results)
{
    ...
    for (var applicationIndex in applications)
    {
        ...
        row.push(application.updated_at);
        ...
        for (var applicationIndex in applications)
        {
            ...
            for (instanceIndex in application)
            {
                ...
                else
                {
                    ...
                    row[12] = (instance.used_memory_in_bytes != null) ? Utilities.convertBytesToMega(instance.used_memory_in_bytes) : 0;
                    row[13] = (instance.used_disk_in_bytes   != null) ? Utilities.convertBytesToMega(instance.used_disk_in_bytes)   : 0;
                    row[14] = (instance.computed_pcpu        != null) ? (instance.computed_pcpu * 100)                              : 0;
                    row[18] = host;
                    row[19] = deaIndex;

                    // Need the specific instance for this row
                    row[20] = {
                                  "application": row[20].application,
                                  "instance": instance,
                                  "space": row[20].space,
...
ApplicationsTab.prototype.clickHandler = function()
{
    ...
    if (selected.length > 0)
    {
        ...
        Utilities.hitchWhenHavingValue(this, table, this.addPropertyRow,        "Created",             Format.formatDate,   row[2] );
        Utilities.hitchWhenHavingValue(this, table, this.addPropertyRow,        "Updated",             Format.formatDate,   row[3] );
        ...
        if (row[5] != "")
        {
            this.addPropertyRow(table, "State", Format.formatString(row[5]));
        }

        if (row[7] != "")
        {
            this.addPropertyRow(table, "Package State", Format.formatString(row[6]));
        }
        ...
        if (row[9] != "")
        {
            var buildpackArray = row[9].split(",");
        ...
        if (row[10] !== "")
        {
            this.addPropertyRow(table, "Instance Index", Format.formatNumber(row[10]));
        }
        ...
        if (row[11] !== "")
        {
            this.addPropertyRow(table, "Services Used", Format.formatNumber(row[11]));
        }
 
        this.addPropertyRow(table, "Memory Used", Format.formatNumber(row[12]));
        this.addPropertyRow(table, "Disk Used",   Format.formatNumber(row[13]));
        this.addPropertyRow(table, "CPU Used",    Format.formatNumber(row[14]));

        this.addPropertyRow(table, "Memory Reserved",  Format.formatNumber(row[15]));
        this.addPropertyRow(table, "Disk Reserved",    Format.formatNumber(row[16]));
        ...
        if (space != null)
        {
            ...
            $(spaceLink).click(function()
            {
                ...
                AdminUI.showSpace(Format.formatString(row[17]));
        ...
        if (row[18] != "")
        {
            var dea = Format.formatString(row[18]);
        ...
        if (instance != null && instance.services != null)
        {
            ...
            for (var serviceIndex = 0; serviceIndex < instance.services.length; serviceIndex++)
            {
                ...
                else
                {
                    ...
                    Utilities.addEmptyElementsToArray(serviceRow, 6);
                }
